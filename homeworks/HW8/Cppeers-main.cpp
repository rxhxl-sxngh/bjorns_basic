...
void processPostsWithHashtags(Network& cppeers) {
  ...
  cout << "Enter tag name: ";
  cin >> tagName;
  vector<Post*> tagsPosts = cppeers.getPostsWithTag(tagName);
  for(unsigned int i = 0; i < tagsPosts.size(); i++) {
    cout << tagsPosts.at(i)->getPostText() << endl;
  }
}
...
void processMostPopularHashtag(Network& cppeers) {
  ...
  for(unsigned int i = 0; i < popularHashtags.size(); i++) {
    cout << popularHashtags.at(i) << endl;
  }
